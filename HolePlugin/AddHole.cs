...
namespace HolePlugin
{
    ...
    public class AddHole : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            {
                Line curve = (duct.Location as LocationCurve).Curve as Line; //отрезок основы трубы
                XYZ point = curve.GetEndPoint(0); //находим точку отсчета 
                ...
                List<ReferenceWithContext> intersections = referenceIntersector.Find(point, direction)
                    ...
                    .Distinct(new ReferenceWithContextElementEqualitComparer())
                    .ToList();
            ...
            {
                Line curve = (pipe.Location as LocationCurve).Curve as Line; //отрезок основы трубы
                XYZ point = curve.GetEndPoint(0); //находим точку отсчета 
                XYZ direction = curve.Direction;
        ...
        public class ReferenceWithContextElementEqualitComparer : IEqualityComparer<ReferenceWithContext>
        {
        public bool Equals(ReferenceWithContext x, ReferenceWithContext y)
        {
                ...
                var xRefeerence = x.GetReference();
                var yRefeerence = y.GetReference();

                return xRefeerence.LinkedElementId == yRefeerence.LinkedElementId
                    && xRefeerence.ElementId == yRefeerence.ElementId;
        }
